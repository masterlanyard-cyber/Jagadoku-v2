...
export default function ThemeToggle() {
  ...
  return (
    <button
      type="button"
      onClick={toggleTheme}
      className="p-2 rounded-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm hover:bg-gray-100 dark:hover:bg-gray-800 transition"
      title={isDark ? "Aktifkan mode siang" : "Aktifkan mode malam"}
      aria-label={isDark ? "Aktifkan mode siang" : "Aktifkan mode malam"}
    >
      {isDark ? (
        <svg className="w-4 h-4 text-gray-700 dark:text-gray-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
